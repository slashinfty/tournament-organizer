...
const Tiebreakers = {
    ...
    compute: (p, t) => {
        ...
        const calcMatchWin = (player, type) => {
            let mwp = (t.winValue * (player.results.length + player.byes)) === 0 ? 0 : player.matchPoints / (t.winValue * (player.results.length + player.byes));
            mwp = isNaN(mwp) ? 0 : mwp;
            if (type === 'magic') return mwp < 0.33 ? 0.33 : mwp;
            else return mwp < 0.25 ? 0.25 : mwp;            
        };
        const calcGameWin = player => {
            let gwp = (t.winValue * (player.results.length + player.byes)) === 0 ? 0 : player.gamePoints / (t.winValue * (player.results.length + player.byes));
            gwp = isNaN(gwp) ? 0 : gwp;
