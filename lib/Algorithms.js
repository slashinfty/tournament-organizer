...
const Algorithms = {
    ...
    swiss: (matches, players, round, maxPoints, sort) => {        
        ...
        let loopCheck = 0;
        let success;
        let indexByePlayer = 0;
        const pairAttempt = (p, u) => {
            let matchNumber = 1;
            let innerLoopCheck = 0;
            let hold = [];
            newMatches = [];            
            do {
                innerLoopCheck++;
                ...
                while (hold.length > 0) {
                    ...
                    if (hold[0].results.findIndex(o => o.opponent === thesePlayers[l].id) === -1) {
                        ...
                        let playersColor = Algorithms.pairCompareSwiss(hold[0], thesePlayers[l]);
                        const newMatch = new Match(newId, round, matchNumber, playersColor);
                ...
                while (thesePlayers.length > 1) {
                    ...
                    while (i < thesePlayers.length) {
                        ...
                        if (thesePlayers[0].results.findIndex(o => o.opponent === thesePlayers[i].id) === -1) {
                            ...
                            let playersColor = Algorithms.pairCompareSwiss(thesePlayers[0], thesePlayers[i]);
                            const newMatch = new Match(newId, round, matchNumber, playersColor);
                            newMatch.active = true;
                            newMatches.push(newMatch);
                            matchNumber++;
                ...
                p = u.reduce((x, y) => y.matchPoints >= p ? x : Math.max(x, y.matchPoints), 0);
            } while ((p >= 0) && (innerLoopCheck < 100));
            if (innerLoopCheck >= 100){
                return false
            }
        };
        do {
            ...
            let byePlayersOrder = [...players];
            //Bye is allowed only once per player
            if (((players.length % 2) !== 0) && (round > 1) && (indexByePlayer < byePlayersOrder.length)){                
                Utilities.byesSort(byePlayersOrder);
                byePlayers.push(byePlayersOrder.splice(indexByePlayer,1)[0]);
            }
            let initialByes = [];
            ...
            if (!success){
                //failed, so I'll try give bye to another player
                indexByePlayer++;
            }
            loopCheck++;
