    ...
    dutch: (matches, players, round, maxPoints) => {
        ...
        const pairAttempt = p => {
            ...
            do {
                ...
                if (collapse && players.reduce((x, y) => y.matchPoints >= p ? x : Math.max(x, y.matchPoints), 0) === 0) {
                    thesePlayers = thesePlayers.concat(players.filter(x => x.matchPoints === 0));
                    p = 0;
                }
                // If no one has the amount of points.
                if (thesePlayers.length === 0) {
